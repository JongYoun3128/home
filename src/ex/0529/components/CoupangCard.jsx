...
function CoupangCard({title,isSubTitle = false}) {
    ...
    return (
        <div className="row-span-1 bg-white min-h-48 border border-collapse border-gray-300">
            <div
            className={`${!isSubTitle ? 'bg-gray-100 border-b border-gray-300' : 'border-b border-b-gray-300'} flex items-center justify-between px-3 py-2`}>
                <h3
                ...
                className={`${!isSubTitle ? 'bg-blue-400 text-white p-1.5 text-xs rounded-md' : 'hidden'}`}>
                    <FaPlus/>
                </button>
            </div>
